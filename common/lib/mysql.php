...
class Mysql {
	...
	public function connect($host, $port, $username, $password) {
		if(!$this->_conn = mysql_connect("{$host}:{$port}", $username, $password)){
	...
	public function selectDb($again = false) {
		if(!mysql_select_db($this->_database, $this->_conn)){
			$again && exit("Could not select database ".$this->_database.": " . mysql_error());
	...
	public function fetch_array($rs, $type = MYSQL_ASSOC) {
		return mysql_fetch_array($rs, $type);
	}

	public function insertId() {
		if(($id = mysql_insert_id($this->_conn))>=1){
			return $id;
		}
		else if($rs = $this->query("SELECT last_insert_id() as id")){
			if($row = $this->fetch_array($rs, MYSQL_NUM)){
				return $row[0];
			}
		}
		return 0;
	...
	public function close() {
		is_resource($this->_conn) && mysql_close($this->_conn);
	}
